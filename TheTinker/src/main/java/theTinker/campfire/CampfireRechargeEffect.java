    ...
    public void update() {
        ...
        if (!didThing) {
            ...
            for (AbstractRelic q : AbstractDungeon.player.relics) {
                if (q instanceof AbstractMachine) {
                    if (q.counter < ((AbstractMachine) q).charges) {
        ...
        if (!relicSelected) {
            if (relicSelectScreen.doneSelecting()) {
                ...
                AbstractMachine blah = (AbstractMachine)AbstractDungeon.player.getRelic(relicSelectScreen.getSelectedRelics().get(0).relicId);
                blah.flash();
                blah.counter = blah.charges;
                if (AbstractDungeon.player.hasRelic(RechargerDrone.ID)) {
                    AbstractDungeon.player.getRelic(RechargerDrone.ID).flash();
                    blah.counter++;
                }
